...
try {
    ...
    switch ($acao) {
        case 'ativar':
            $sql = "UPDATE user SET identity_verified = 1, updated_at = NOW() WHERE user_id = ?";
        ...
        case 'desativar':
            $sql = "UPDATE user SET identity_verified = 0, updated_at = NOW() WHERE user_id = ?";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$user_id]);
    ...
    // Tentar inserir log (pode falhar se a tabela não existir)
    try {
        $log_stmt->execute([$_SESSION['usuario_id'], $acao, $log_detalhes, $user_id]);
