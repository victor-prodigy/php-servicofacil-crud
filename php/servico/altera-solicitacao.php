...
try {
  ...
  // 📝 Verificar se ID da solicitação foi fornecido
  ...
  $sqlVerificar = "SELECT request_id FROM service_request WHERE request_id = ? AND cliente_id = ?";
  $stmtVerificar = $pdo->prepare($sqlVerificar);
  $stmtVerificar->execute([$solicitacaoId, $clienteId]);
  $solicitacaoExiste = $stmtVerificar->fetch();
  ...
  try {
    ...
    // ✨ Sucesso
    enviarResposta(true, 'Solicitação atualizada com sucesso!', [
    ...
    enviarResposta(false, 'Erro ao atualizar no banco de dados: ' . $e->getMessage());
  }
} catch (Exception $e) {
  error_log("Erro em altera-solicitacao.php: " . $e->getMessage());
